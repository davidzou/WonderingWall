...
public abstract class SDSQLiteOpenHelper {
		...
		try {
			...
			if (mName == null) {
				...
				if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
					String path = setPath(mName).getAbsolutePath();
					Log.d("db", "" + path);
	...
	public synchronized SQLiteDatabase getReadableDatabase() {
		...
		try {
			mIsInitializing = true;
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				String path = setPath(mName).getPath();
				...
				db = SQLiteDatabase.openDatabase(mName, mFactory, SQLiteDatabase.OPEN_READWRITE);
	...
	private File setPath(String name) {
		String path = Environment.getExternalStorageDirectory().toString();
		if (!TextUtils.isEmpty(saveAsPath())) {
			File dir = new File(path + saveAsPath());
			if (!dir.exists()) {
				if (!dir.mkdir()) {
	...
	/**
	 * Description（描述）: 另存为路径, 不设置模式在根目录下.<br/>
	 * Conditions（适用条件）: <br/>
	 * Execution flow（执行流程）:<br/>
	 * Usage（用法）:<br/>
	 * Cautions（注意事项）: 如果有数据返回则返回的格式必须为"/for/bar", 必须有“/”开头.<br/>
	 * 
	 * @author DavidZou
	 * @return
	 */
	public abstract String saveAsPath() ;
